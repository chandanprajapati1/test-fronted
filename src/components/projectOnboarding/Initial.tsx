...
const InitialStep: React.FC = () => {
    ...
    const handleNextClick = useCallback(() => {
        if (selectedOption) {
            ...
            if (selectedOption == 'Carbon Registry') {
                ...
                router.replace(`${Routes.PuroEarthProject}`);
    ...
    return (<div className="flex flex-col items-center mx-auto bg-white px-5 sc-sm:px-3xl py-6 gap-6">
            <div className="text-center gap-1">
                <h1 className="text-f-5xl font-light text-neutral-1400">Project Onboarding</h1>
                <p className="text-base text-neutral-1200">Select the option below for your project onboarding</p>
            </div>
            <div className="flex flex-wrap justify-center gap-6 py-16 sc-sm:py-4xl">
                {onboardingOptions.map((option, index) => (<button
                        key={index}
                        className="p-6 rounded-2xl bg-white border shadow-sm cursor-pointer hover:border-brand1-500 focus:outline-none focus:border focus:border-brand1-500 focus:border-1 max-w-[333px]"
                        onClick={() => handleOptionSelect(option.title)}
                    >
                        <div className="flex justify-between items-start h-full">
                            <div className="flex flex-col justify-start text-left gap-3">
                                <p className="text-f-s font-bold uppercase text-tertiary">{option.title}</p>
                                <p className="text-f-2xl font-light text-neutral-1000">{option.description}</p>
                            </div>
                            <div
            ...
            <hr className="border-t border-1 w-full border-neutral-200"/>
            <h2 className="text-neutral-1400 text-f-3xl font-light">Registries</h2>
            <div className="flex flex-col sc-xs:flex-wrap sc-xs:flex-row gap-6  justify-center">
                {registries.map((registry, index) => (<div key={index}
                        className={` `}>
                        <button
                            key={registry.id}
                            className={`min-w-[220px] p-6 rounded-2xl bg-white border ${selectedOption === registry.name ? 'border-primary' : 'border-neutral-100'} shadow-sm overflow-hidden cursor-pointer hover:border-brand1-500 focus:outline-none focus:border focus:border-brand1-500 focus:border-1 `}
            ...
            <div className="flex justify-center mt-4 ">
                <button
                    className={`flex items-center justify-center gap-l pr-[20px] pl-xl py-l rounded-lg ${selectedOption ? 'bg-brand1-500 text-white' : 'bg-neutral-400 text-black'} transition-colors duration-200 focus:outline-none focus:border focus:border-brand1-500 focus:border-1`}
