...
import AdminLayout from "@/components/layouts/adminWithoutAuth";
...
const Page = () => {
    ...
    return (<AdminLayout>
        {loading ? (
            <Loader />
        ) : <div className='text-neutral-1400 flex flex-col  bg-gray-100'>
                <div className='w-full h-[522px] relative'>
                    {headerProject.length > 0 && (<ProjectHeader data={headerProject[0]} activeToken={activeToken}/>)}
                    <SearchFilterBar setParamsData={setParamsData}/>
                </div>

                <div className='mt-56 sc-xs:mt-32 sc-md:mt-8 flex flex-col w-full justify-center py-m px-6  max-w-screen-sc-2xl mx-auto'>

                    <div className='grid grid-cols-1 sc-sm:grid-cols-2 gap-xl justify-between'>
                        {secondaryProjectList.map((projectData, index) => (
                            <ProjectCard key={index} data={projectData} activeToken={activeToken}/>))}
                    </div>

                    {projectList.map((projectArr: any, index: number) => (
                        <div key={index} className='py-m grid grid-cols-1 sc-sm:grid-cols-2 sc-xl:grid-cols-4 justify-between gap-xl'>
                            {projectArr.map((projectData: any, innerIndex: number) => (
                                <ProjectCardMini key={innerIndex} data={projectData} activeToken={activeToken} len={projectArr.length}/>))}
                        </div>))}

                    {enterLoad && hasMore && (<div ref={observerRef} className="h-10 mt-4 text-gray-600 ">
